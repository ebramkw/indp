...
int dsvd(float a[M][N], int m, int n, float w[N], float v[N][N])
{
...
/* Householder reduction to bidiagonal form */
    ...
    Sort_by_Decreasing_Singular_Values(n, w, v);

    return(1);
}

void Sort_by_Decreasing_Singular_Values(int ncols,
                                float singular_values[N], float singular_vectors[N][N])
{
   int i,j,max_index;
   float temp;
   ...
   for (i = 0; i < ncols - 1; i++)
   {
      ...
      for (j = 0; j < ncols; j++) {
         temp = singular_vectors[j][i];
         singular_vectors[j][i] = singular_vectors[j][max_index];
         singular_vectors[j][max_index] = temp;
      }
   } 
}